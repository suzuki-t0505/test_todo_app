defmodule TestTodoAppWeb.AccountAuthTest do
  use TestTodoAppWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias TestTodoApp.Accounts
  alias TestTodoAppWeb.AccountAuth
  import TestTodoApp.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, TestTodoAppWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_account/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      TestTodoAppWeb.Endpoint.subscribe(live_socket_id)
